...
if (isset($_POST['consultar'])) :
    ...
    $id = $_POST['medico'];
    $sql = "select `trabalhogildo`.`medico`.*, `c`.`data`, `c`.`motivo`, `c`.`historico`, `c`.`hora`
            from `trabalhogildo`.`medico` inner join `trabalhogildo`.`consulta` c on `medico`.`idmedico` = `c`.`idmedico`
            where `medico`.`idmedico`= $id ;"; //pegar todos elementos (fez o select) pegar id do medico
